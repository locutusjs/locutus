function bcsub (left_operand, right_operand, scale) {
    ...
    var d = "";
    var type="";
    var AnswerArray1 = left_operand.split("."); // Split into an array
    var AnswerArray2 = right_operand.split("."); // Split into an array
    ...
    left_operand = AnswerArray1[0] + "." + AnswerArray1[1];
    right_operand = AnswerArray2[0] + "." + AnswerArray2[1];
    if (AnswerArray1[1].length > AnswerArray2[1].length) {
        right_operand += bc_makestr("0",AnswerArray1[1].length-AnswerArray2[1].length+"");
    }
    if (AnswerArray2[1].length > AnswerArray1[1].length) {
        left_operand += bc_makestr("0",AnswerArray2[1].length-AnswerArray1[1].length+"");
    }
    if (AnswerArray1[0].length > AnswerArray2[0].length) {
        right_operand = bc_makestr("0",AnswerArray1[0].length-AnswerArray2[0].length+"") + right_operand;
    }
    if (AnswerArray2[0].length > AnswerArray1[0].length) {
        left_operand = bc_makestr("0",AnswerArray2[0].length-AnswerArray1[0].length+"") + left_operand;
    }
    if (!d && (bc_is_equal(left_operand,right_operand)=="TRUE")) {
        d= "0";
    }
    if (!d && left_operand.charAt(0)!="-" && right_operand.charAt(0)=="-") {
        right_operand=right_operand.substring(1,right_operand.length);
        d = this.bcadd(left_operand,right_operand, 10);
    }
    if (!d && left_operand.charAt(0)=="-" && right_operand.charAt(0)=="-") {
        right_operand=right_operand.substring(1,right_operand.length);
        left_operand=left_operand.substring(1,left_operand.length);
        var TemporaryVariable = right_operand;
        right_operand = left_operand;
        left_operand = TemporaryVariable;
    }
    if (!d && left_operand.charAt(0) == "-" && right_operand.charAt(0) !="-") {
        left_operand=left_operand.substring(1,left_operand.length);
        d = "-" + this.bcadd(left_operand,right_operand, 10);
    ...
    if (!d) {
        ...
        if (bc_is_less_than(left_operand,right_operand)=="TRUE") {
            var TempVariable = left_operand;
            left_operand = right_operand;
            right_operand = TempVariable;
            NegativeTag="-";
        }
        var BorrowFlag="0";
        for (counter = left_operand.length-1; counter > -1; counter--) {
            if (left_operand.charAt(counter) ==".") {
                d = "." + d;
            } else {
                var left_operandPiece = parseInt(left_operand.charAt(counter));
                var right_operandPiece = parseInt(right_operand.charAt(counter));
                if (BorrowFlag == 1) {BorrowFlag = 0; left_operandPiece = left_operandPiece -1}
                if (left_operandPiece < 0) {left_operandPiece = 9; BorrowFlag = 1}
                if (left_operandPiece < right_operandPiece) {
                    left_operandPiece = left_operandPiece + 10;
                    BorrowFlag = 1;
                }
                d = (left_operandPiece - right_operandPiece) + d;
            }
        }
        d = bc_trimzeros(d);
        d = NegativeTag + d;
    }
    return bc_trunc(d, scale);
}