function token_get_all(source) {
    ...
    getCurrentCasting = function (start) {
        ...
        if (match && match[1]) {match[1]=match[1].toLowerCase();}
        return match;
    },
    ...
    getCurrentWhitespaces=function(start,mode) {
        ...
        if (!mode) {
            ...
            if (ascii==10 || (ascii==13 && source.charCodeAt(start+1)==10)) {
                return (ascii==13 ? source[start]+source[start+1] : source[start]);
            }
            else {return "";}
        }
        else {
    ...
    for (var i=0;i<source.length;i++) {
        ...
        var ch=source[i],ASCII=source.charCodeAt(i);
            ...
            switch (bufferType) {
                ...
                case "singleQuote":
                    ...
                    if (bufferType=="singleQuote" || (ch!="$" && ch!="{") || isEscaped(i)) {
                        ...
                        if (!hdlabel && ((ch=="'" && bufferType=="singleQuote")||(ch=='"' && bufferType=="doubleQuote")) && !isEscaped(i)) {
            ...
            if (ch=="$") {
                ...
                if (source[i+1]=="{") {
                    ...
                    if (nextCharWord) {
                        ...
                        var afterChar=source[i+nextCharWord[0].length+2];
                        if (afterChar=="}") {
                        ...
                        //${a[0]},${a[b]}
                            ...
                            if (nextNextCharWord && source[i+nextCharWord[0].length+3+nextNextCharWord[0].length]=="]" &&
                                source[i+nextCharWord[0].length+3+nextNextCharWord[0].length+1]=="}") {
                                toInsert.push([tokens.T_DOLLAR_OPEN_CURLY_BRACES,"${"]);
                                toInsert.push([tokens.T_STRING_VARNAME,nextCharWord[0]]);
                                toInsert.push("[");
                ...
                //$a
                    ...
                    if (nextCharWord) {
                        ...
                        if (source[i+1]=="[") {
                            ...
                            nextCharWord=getCurrentWord(i+2);
                            if (nextCharWord && source[i+nextCharWord[0].length+2]=="]") {
                                toInsert.push("[");
        ...
        var couple=ch+source[i+1],triplet=couple+source[i+2],insString;
        ...
        switch (couple) {
            ...
            case "<<": 
                ...
                if (source[i+2]=="<" && (nextCharWord=getHeredoc(i+3))) {
            ...
            case "<?":
                insString=couple;
                if (couple=="<?" && source[i+2]=="p" && source[i+3]=="h" && source[i+4]=="p") insString+="php";
            ...
            case "/*":
                buffer=couple;
                if (source[i+2]=="*" && /\s/.test(source[i+3])) {
                    bufferType="DOCComment";
                    buffer+=source[i+2]+source[i+3];
                    i+=2;
                }
