module.exports = function xmlwriter_open_memory ()  {
  ...
  function LOCUTUS_Resource(type, id, opener) { // Can reuse the following for other resources, just changing the instantiation
    // See http://php.net/manual/en/resource.php for types
    this.type = type;
    this.id = id;
    this.opener = opener;
  }
  LOCUTUS_Resource.prototype = {
  ...
  XMLWriter.prototype = new LOCUTUS_Resource('xmlwriter', this.php_js.resourceIdCounter, 'xmlwriter_open_memory'); // is the first argument the right resource type for XMLWriter?

  // XMLWriter.prototype.constructor = XMLWriter; // we may need to keep the constructor as LOCUTUS_Resource for type checking elsewhere; can use type property instead
