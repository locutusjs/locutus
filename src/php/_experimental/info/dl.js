module.exports = function dl (library)  {
  ...
  // %        note 1: The locutus.extensions_detect_files custom option requires privileges (and Mozilla at present)
  ...
  this.php_js.ini['locutus.loaded_extensions'] = this.php_js.ini['locutus.loaded_extensions'] || {};
  this.php_js.ini['locutus.loaded_extensions'].local_value = this.php_js.ini['locutus.loaded_extensions'].local_value || [];
  if (!this.php_js.ini['locutus.loaded_extensions'].local_value.indexOf) {
    this.php_js.ini['locutus.loaded_extensions'].local_value.indexOf = indexOf;
  }
  if (this.php_js.ini['locutus.loaded_extensions'].local_value.indexOf(library) === -1) {
    this.php_js.ini['locutus.loaded_extensions'].local_value.push(library);
  }
  ...
  subdirs = _ini_get('locutus.extension_dir_subdirs');
  skipErrors = _ini_get('locutus.dl_skip_errors');
  ...
  if (_ini_get('locutus.extensions_individual_files')) {
    if (_ini_get('locutus.extensions_detect_files')) {
...
//*/
    ...
    if (libraries.indexOf(library) === -1 && !_ini_get('locutus.dl_allow_individual_funcs')) {
    ...
    for (i = 0; i < ext_dir.length && !ret; i++) { // Stop after success
      basepath = subdirs && !_ini_get('locutus.dl_allow_individual_funcs') ? ext_dir[i] + sep + library + sep : ext_dir[i] + sep;
