...
locutusUtil.opener = function (fileOrName, cb) {
  ...
  if (fileOrName.indexOf('/') === -1) {
    ...
    throw new Error('gimme full path')
  }
...
cli.injectweb = function (args, options) {
  var self = this
  var pattern = __src + '/' + options.language + '/' + options.category + '/' + options.name + '.js'
  ...
  self.glob(pattern, function (err, params, file) {
    ...
    var webfuncPath = __root + '/website/_functions/' + params.language + '/' + params.category + '/' + params.func_name + '.html'
    var data = {
