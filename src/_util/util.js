...
Util.prototype.injectweb = function (args, options) {
  ...
  self.glob(pattern, function (err, params, file) {
    ...
    var data = {
      ...
      dependencies: _.flattenDeep(params.headKeys['depends on'] || []),
      authors: authors || {},
...
Util.prototype.test = function (args, options) {
  ...
  var knownFailures = fs.readFileSync(self.__src + '/' + options.language + '/known-failures.txt', 'utf-8').trim('').split('\n')
...
Util.prototype._test = function (params, cb) {
  ...
  self.loadMultiple(self.injectDependencies, function (err, paramsMultiple) {
    ...
    for (var path in paramsMultiple) {
      codez.unshift(paramsMultiple[path].code)
    }
    ...
    if (!params.func_name) {
      return cb(new Error('No func_name in ' + JSON.stringify(params)))
    }
    if (!params.headKeys) {
      return cb(new Error('No headKeys in ' + params.func_name))
    }
    if (!params.headKeys.example) {
      return cb(new Error('No example in ' + params.func_name))
    }
