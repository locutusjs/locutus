module.exports.ascii_letters = require('./ascii_letters')
module.exports.ascii_lowercase = require('./ascii_lowercase')
module.exports.ascii_uppercase = require('./ascii_uppercase')
module.exports.capwords = require('./capwords')
module.exports.digits = require('./digits')
module.exports.hexdigits = require('./hexdigits')
module.exports.octdigits = require('./octdigits')
module.exports.printable = require('./printable')
module.exports.punctuation = require('./punctuation')
module.exports.whitespace = require('./whitespace')
