...
cli.injectweb = function (args, options) {
  ...
  self.glob(pattern, function (err, params, file) {
    ...
    var webfuncPath = __root + '/website/_functions/' + cat + '/' + params.func_name + '.md'
    var data = {
      params: {
        headKeys: params.headKeys
      },
    ...
    buf += '\n'
    buf += '<!-- WARNING! This file is auto generated by `npm run web:inject`, do not edit by hand -->\n'
    buf += '\n'
    buf += 'Here\'s what a JavaScript equivalent for PHP’s array_chunk might look like' + '\n'
    buf += '\n'
    ...
    if (params.headKeys.note) {
      buf += '## Notes'
      buf += '\n'
      buf += '- ' + params.headKeys.note.join('\n- ') + '\n'
      buf += '\n'
    }

    if (params.headKeys.example) {
      params.headKeys.example.forEach(function (example, i) {
        var result = params.headKeys.example[i]
        buf += '## Example ' + (i + 1) + '\n'
        buf += '\n'
        buf += '{% highlight javascript %}' + '\n'
        buf += example + '\n'
    ...
    keys.forEach(function (key) {
      if (params.headKeys[key]) {
        buf += '\n'
        buf += key.charAt(0).toUpperCase() + key.substring(1).toLowerCase() + '\n'
        buf += '\n'
        buf += '- ' + params.headKeys[key].join('\n- ') + '\n'
        buf += '\n'
      }
    })
