function ldap_bind (link_id, bind_rdn, bind_pass, successCb, errorCb) {
    ...
    // %        note 1: We break somewhat with the PHP API by necessity in order to allow
    // %        note 1: asynchronous callbacks, having the last two arguments being a
    // %        note 1: successful callback and an error callback respectively, with the
    // %        note 1: latter being passed the error and result and the former being
    // %        note 1: passed the result only.
