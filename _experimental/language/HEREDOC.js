function HEREDOC (xmllist) {
	...
	// %          note 3: To include multi-line whitespace, must encapsulate in an XMLList (see example), the literal form being easiest and reminiscent of HEREDOC angled brackets
	// %          note 4: As per E4X rules, {} can be used to encapsulate variables or JavaScript expressions within the HEREDOC
	// %          note 5: Could be renamed HERE, etc., as the HEREDOC naming is not fixed in PHP either
	// %          note 6: Unfortunately this approach will not work to simulate the PHP 5.3+ NOWDOC; you must escape any literal curly brackets manually with &#x7B and &#x7D;
