        ...
        switch(instruction) {
            ...
            case 'C': //unsigned char
             ...
             case 'd': // double (machine dependent size and representation)
                ...
                for (i = 0; i < quantifier; i++) {
                    var number = arguments[argumentPointer], bias = Math.pow(2, exponentBits - 1) - 1, minExp = -bias + 1, maxExp = bias, minUnnormExp = minExp - precisionBits, status = isNaN(n = parseFloat(number)) || n == -Infinity || n == +Infinity ? n : 0, exp = 0, len = 2 * bias + 1 + precisionBits + 3, bin = [](len), signal = (n = status !== 0 ? 0 : n) < 0, n = Math.abs(n), intPart = Math.floor(n), floatPart = n - intPart, lastBit, rounded, j, tmpResult;
                    for(i = len; i; bin[--i] = 0)
